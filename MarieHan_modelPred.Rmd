...
---
Sources for ideal rain fall
sources for ideal and detrimental temperature
Things about cherry blossoms, how they are impacted by global warming and what that indicates about general climate trends. 

Before it's able to render, must load the original cherry dataframe into this markdown file so it'll compile all cute and stuff. 
...
Getting the data, loading it into our environment: 
...
Location data, putting into one dataframe, daily information
```{r}
...
Now to make our data usable in the model and prediction, collapsing and summarizing it into a year
```{r}
...
Make sure to include that the too hot indicator was insignificant due to it being fall and winter time during predictions. Unlikely to have a random heatwave but not unheard of. 
```{r}
model1_TEST <- cherry |>
