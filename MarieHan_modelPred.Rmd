...
Cherry_dayTEST <- bind_rows(Cherry_day, vancouver_extra)
```
...
Cherry_weekTEST <- Cherry_dayTEST %>%
  filter(month >= 10 | month <= 2) %>% 
  mutate(week = isoweek(Date)
  ) %>%
  group_by(location, bloom_year, week) %>%
...
Cherry_YearTEST <- Cherry_weekTEST %>%
  group_by(location,bloom_year) %>%
  summarize(
    precipitation = sum(PRCP_indicator),
    avg_TMAX = mean(max_TAVG),
...
model1_TROUBLESHOOT <- cherry |>
  left_join(Cherry_YearTEST,
             by = c("location", "year")) |>
  lm(formula = bloom_doy ~ year * location +  AVG + TooCold+TooHot +precipitation )
  

summary(model1_TEST)
summary(model1_TROUBLESHOOT)
```
...
```{r}
#TROUBLE SHOOT PREDICTIONS FOR VANCOUVER 
cherry_gridTROUBLESHOOT <- expand_grid(location = unique(cherry$location),
                           year = 1990:2026) |> 
  inner_join(Cherry_YearTEST,
             by = c("location", "year"))

predictions3 <- cherry_gridTROUBLESHOOT |> 
  mutate(pred_bloom = predict(model1_TROUBLESHOOT, newdata = cherry_gridTROUBLESHOOT))
...
```{r}
vancouver_extra <- vancouver_binded %>%
  mutate(
    Date = as.Date(date),
    year = year(Date),
    TMIN = tmin,
